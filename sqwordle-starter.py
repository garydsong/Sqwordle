...
class Sqwordle:
    def __init__(self, letters=7, logo=logo):
        self._letters = letters
        self._logo = logo
        self._guess = ''
        self._guesses = [[] for x in range(8)]
        self._tries = 0
    ...
    def play_game(self):
        ...
        while self._tries < 7:
            ...
            if self._guess == self._game_word:
                print(f"You've done it! {self._tries}/7 tries. The pokemon was {self._game_word.upper()}")
                return
        print(f"You've ran out of tries. The pokemon was {self._game_word.upper()}")
